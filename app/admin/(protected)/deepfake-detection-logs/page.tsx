...
export default async function DeepfakeDetectionLogsPage() {
  const logs = await getVerificationLogs({ source: "deepfake", limit: 50 });

  return (
    <div className="space-y-6">
      <div className="rounded-[28px] border border-white/60 bg-white/90 p-6 shadow-glass">
        <p className="text-sm uppercase tracking-[0.25em] text-slate-400">Deepfake Detection Logs</p>
        <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Review synthetic-image risk screening attempts.</h1>
        <p className="mt-3 max-w-3xl text-slate-500">
          The screening log captures authenticity score, risk notes, matched employee identity, and timestamp for every AI risk review.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {logs.length === 0 ? (
          <div className="rounded-[28px] border border-white/60 bg-white/90 px-6 py-12 text-center text-slate-500 shadow-glass">
            No deepfake screening logs yet.
          </div>
        ) : (
          logs.map((log) => (
            <div key={String(log._id)} className="rounded-[28px] border border-white/60 bg-white/90 p-6 shadow-glass">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                ...
                >
                  {log.status === "valid" ? "Cleared" : "Risk Detected"}
                </span>
              </div>

              <div className="mt-5 grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl bg-slate-50 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Authenticity Score</p>
