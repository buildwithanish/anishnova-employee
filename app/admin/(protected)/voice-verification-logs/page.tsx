...
export default async function VoiceVerificationLogsPage() {
  const logs = await getVerificationLogs({ source: "voice", limit: 50 });

  return (
    <div className="space-y-6">
      <div className="rounded-[28px] border border-white/60 bg-white/90 p-6 shadow-glass">
        <p className="text-sm uppercase tracking-[0.25em] text-slate-400">Voice Verification Logs</p>
        <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Review voice declaration verification attempts.</h1>
        <p className="mt-3 max-w-3xl text-slate-500">
          Every voice verification workflow is recorded with confidence, transcript notes, matched employee identity, and audit time.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {logs.length === 0 ? (
          <div className="rounded-[28px] border border-white/60 bg-white/90 px-6 py-12 text-center text-slate-500 shadow-glass">
            No voice verification logs yet.
          </div>
        ) : (
          logs.map((log) => (
            <div key={String(log._id)} className="rounded-[28px] border border-white/60 bg-white/90 p-6 shadow-glass">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                ...
                >
                  {log.status === "valid" ? "Voice Verified" : "Needs Review"}
                </span>
              </div>
